function errors=fsk_noncoherent_errors(bps,Nsymb,ns,EbNo)
%% Input parameters
...
pwelch(s,[],[],[],1/Ts); title('32-FSK non-coherent');
...
for k=1:length(s)/ns
    ...
    for i=1:M
        th=rand();
        si=sin(2*pi*f(i)*tk+2*pi*th);
        sq=cos(2*pi*f(i)*tk+2*pi*th);
        smi(i)=sum(sk.*si);
        smq(i)=sum(sk.*sq);
        sm(i)=sqrt(smi(i)^2+smq(i)^2);
    end
    [m,l]=max(sm);
    xr=[xr;de2bi(l-1,bps)];
end
